...
def test_post_prune_cuts_deep_and_low_sample_positions():
    builder = make_builder()
    builder.config.postprune_max_depth = 1
    builder.config.postprune_min_games = 50
    ...
    keep_board = chess.Board()
    deep_board = chess.Board()
    deep_board.push(chess.Move.from_uci("e2e4"))
    low_board = chess.Board()
    low_board.push(chess.Move.from_uci("d2d4"))

    keep_child = RepertoireNode(
        board=keep_board.copy(),
        fen=keep_board.fen(),
        key="keep",
        is_player_turn=False,
        min_player_depth=1,
        terminal_advantage=0.25,
        position_stats={"lichess": {"white": 30, "draws": 20, "black": 20}},
    )

    deep_child = RepertoireNode(
        board=deep_board.copy(),
        fen=deep_board.fen(),
        key="deep",
        is_player_turn=False,
        min_player_depth=2,
        terminal_advantage=0.1,
        position_stats={"lichess": {"white": 100, "draws": 50, "black": 50}},
    )

    low_sample_child = RepertoireNode(
        board=low_board.copy(),
        fen=low_board.fen(),
        key="low",
        is_player_turn=False,
        min_player_depth=1,
        terminal_advantage=0.05,
        position_stats={"lichess": {"white": 10, "draws": 5, "black": 5}},
    )
    ...
    root_node.edges = [
        RepertoireEdge(
            ...
            move_san="Nf3",
            stats=MoveStats("g1f3", "Nf3", 1, 0, 0, 1),
            resulting_depth=1,
            comment="",
            termination_reason=None,
            is_terminal=False,
        ...
        RepertoireEdge(
            ...
            move=chess.Move.from_uci("e2e4"),
            move_san="e4",
            stats=MoveStats("e2e4", "e4", 1, 0, 0, 1),
            resulting_depth=2,
            comment="",
            termination_reason=None,
            is_terminal=False,
        ...
        RepertoireEdge(
            parent=root_node,
            child=low_sample_child,
            move=chess.Move.from_uci("d2d4"),
            move_san="d4",
            stats=MoveStats("d2d4", "d4", 1, 0, 0, 1),
            resulting_depth=1,
            comment="",
            termination_reason=None,
            is_terminal=False,
    ...
    builder.post_prune([root_node])

    keep_edge, deep_edge, low_edge = root_node.edges

    assert not keep_edge.is_terminal
    assert deep_edge.is_terminal
    assert deep_edge.child is None
    assert "Post-pruned" in (deep_edge.termination_reason or "")
    assert pytest.approx(0.1) == deep_edge.terminal_advantage

    assert low_edge.is_terminal
    assert low_edge.child is None
    assert "Post-pruned" in (low_edge.termination_reason or "")
    assert pytest.approx(0.05) == low_edge.terminal_advantage
